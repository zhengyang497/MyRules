...
//
//   node scripts/myrules-board.mjs add --title "..." [--body "..."] [--focus]
//   node scripts/myrules-board.mjs patch --id item-1 --status done [--set focus=true]
//   node scripts/myrules-board.mjs server

import { readFileSync } from "fs"
import { spawn } from "child_process"
import { findItem, patchItem, readItems, writeItem } from "./myrules-board-io.mjs"
...
function usage(code = 1) {
  console.error(`用法:
  node scripts/myrules-board.mjs summary
  node scripts/myrules-board.mjs next
  node scripts/myrules-board.mjs get --id <id>
  node scripts/myrules-board.mjs add --title <标题>
       [--body <文本>] [--body-file <path>]
       [--placement <改承诺|新承诺|只记工作>] [--goal <设计目标路径>] [--focus]
  node scripts/myrules-board.mjs patch --id <id> [--status todo|partial|done|wont]
...
function cmdServer() {
  const child = spawn(process.execPath, ["scripts/myrules-board-server.mjs"], {
    stdio: "inherit",
    cwd: process.cwd(),
  })
