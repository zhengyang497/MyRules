...
- **Target files**: `coding-standards` (primary), `behavior` (architecture proposals), `planning` (new-project skeleton)
...
| 下任务时标明动哪一层（UI / state / logic / IO） | **可部署** (`behavior`) | 可观察；删掉范围声明更易模糊 |
...
硬性格式可写入 planning / behavior 候选（见 merge-queue）。
...
See `merge-queue/into-coding-standards.md` and `merge-queue/into-behavior.md` (layer-scoping bullets).
