...
- **Target files**: `planning` (primary), `behavior`, `review` (optional), overlaps `coding-standards` / `software-layering-patterns.md`
...
| 范围用验收裁剪，不按「文件数少」炫技式最小化 | **可部署** (`behavior`) | 与现有「Minimize scope」去重：改成 must/optional |
...
See merge-queue: `into-planning.md`, `into-behavior.md`, `into-review.md`, and strengthen `into-coding-standards.md` if needed.

## Merge notes

- Overlaps: `software-layering-patterns.md`（层与桥）、`agent-controllability.md`（先定义完成标准）、现有 `planning.md`「decompose + acceptance」、`behavior.md`「Minimize scope of changes」。
