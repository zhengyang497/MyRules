...
<!-- empty after 2026-09-05 prune -->

## Merged

- [x] 2026-09-05 — 删「用对方这轮的语言写……不要先写成英文再译」整条。语气、具体名词、中文组织改由自然书写条覆盖。
  - Source: 用户点名删除该句（2026-09-05）
  - Merged: 2026-09-05 into `rules/user/communication.md`

- [x] 2026-09-05 — 新增自然书写条：每句话遵循人类语言习惯；专业场景中自然写，句式有长短变化；禁止电报体、AI 模板、机械排比、重复总结、空泛过渡、过度分点、不必要的抽象词，以及为显得专业而写的生硬冗长句。双极：不要电报体/模板腔，也不要为通顺改成口语或删掉必要细节（后半仍由「不要为了显得简单而删掉必要细节」管）。
  - Source: 用户要求沟通规则新增一条（2026-09-05）
  - Merged: 2026-09-05 into `rules/user/communication.md`

- [x] 2026-09-05 — 沟通规则只留五句，其余删除。保留：读过的文件不当成对方已知 + 代码名第一次用完整句子说明再写原名；用对方这轮语言、名词具体、不要梗和空泛哲理、不要为简单删细节、按中文组织句子；设计/计划弱假设要反驳，改一行就直接做；先说结果再补依据；动手前写出假定，多种理解摊开，不清楚就问。删掉：少用代词、信达雅与整句检查、分点语体、比喻/点名仪式、篇幅随问题。
  - Source: 用户看沟通规则正文后点名只留这五句（2026-09-05）
  - Merged: 2026-09-05 into `rules/user/communication.md`


- [x] 2026-08-19 — 点名句：「说它做什么」改成完整句子说明它管什么，禁止另起两三个字的短名当标签。真实失败：页头 `status` / `confidence` 被收成短名后反复当标签。删「外号先定义再当标签」后，残留的「普通中文说它做什么」仍在造外号。双极：（1）第一次用完整句子说明它管什么，再写原名；（2）之后用原名，不做成词条，也不把篇幅条改成一律整句。计划/文档里已有短名，对对方说话时仍展开，除非对方这轮自己用了。整句检查仍只管翻译腔，不改。
  - Source: 用户问 global-104 短名为何不是整句话，确认改缓存并同步（2026-08-19）
  - Merged: 2026-08-19 into `rules/user/communication.md`

- [x] 2026-08-19 — 删表第 6 条正反例（股票代码表 / `.llm-wiki` 那对）。整句检查里「和 X 一样……两端读同一份」仍留作说明书腔样本。
  - Source: 用户看沟通规则表后要求删第 6 条（2026-08-19）
  - Merged: 2026-08-19 into `rules/user/communication.md`

- [x] 2026-08-19 — 表第 2 条（用对方这轮语言）与第 3 条（按中文组织、信达雅）并成一条。正文未改写，只去掉中间空行。整句检查、分点语体仍另段。
  - Source: 用户看沟通规则表后要求合并（2026-08-19）
  - Merged: 2026-08-19 into `rules/user/communication.md`

- [x] 2026-08-19 — 删「术语/外号先定义再当标签」整句（含「对方用过的词不必再定义」「不要一上来就抛这些名字」「不要做成目录」）。真实失败：把界面「原始资料」收成外号「源」。该句把外号合法化了。双极：（1）不再另起短名当标签；（2）文件里的名字仍要让人看得懂——留给「指名按界面」「代码名先说做什么」。不要做成目录改由分点语体管。
  - Source: 用户看沟通规则表后要求删（2026-08-19）
  - Merged: 2026-08-19 into `rules/user/communication.md`

- [x] 2026-08-19 — 进度/收尾指代条并进第一条，收成「少用代词」。删掉点名仪式和「禁止一切代词是相反的错」整句。双极：少用，不是一律不用。
  - Source: 用户看六份规则表（2026-08-19）
  - Merged: 2026-08-19 into `rules/user/communication.md`

- [x] 2026-08-19 — 删「句子宜短」。与信达雅、前后句要接得上冲突：宜短会收成碎句或说明书条目。名词要具体仍保留。
  - Source: 用户看六份规则表后要求删（2026-08-19）
  - Merged: 2026-08-19 into `rules/user/communication.md`
