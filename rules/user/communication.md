...
- Prefer plain language; define technical or coined terms first; when explaining mechanisms, state trade-offs - not slogan-level summaries.
...
- 解释类问题都要走完四步，不限于「为什么做」。包括还差什么、结论是什么、会不会坏、还要不要等、分几步做。每一步用这个项目里用户能碰到的例子，不要只丢结论：
  ...
  2. 现在程序实际怎么处理；
  3. 如果改了，用户能看见什么变化（好的和坏的都说）；
  4. 为什么现在改或不改。
- 先讲用户要做什么、能看见什么。看板编号、段名、字段名、以及门闩/缺口等黑话放后面；第一次出现必须用大白话定义。禁止用「还差两步」这种目录句代替每一步的展开。
- 禁止把详细解释压成口号、对照表或两三句收束。默认带中间过程，不要先删再等下一轮补。
